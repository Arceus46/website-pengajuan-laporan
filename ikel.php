...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_keluhan'])) {
    ...
    if ($judul === '' || $isi === '') {
        ...
        // Auto generate id_keluhan (K001, K002, ...)
        $qID = mysqli_query($conn, "SELECT id_keluhan FROM tb_keluhan ORDER BY id_keluhan DESC LIMIT 1");

        if (mysqli_num_rows($qID) > 0) {
            $row = mysqli_fetch_assoc($qID);
            $lastID = $row['id_keluhan'];
            $num = intval(substr($lastID, 1));
            $num++;
            $id_keluhan = 'K' . str_pad($num, 3, '0', STR_PAD_LEFT);
        } else {
            $id_keluhan = 'K001';
        }
        ...
        $stmt = mysqli_prepare($conn,
            "INSERT INTO tb_keluhan 
            (id_keluhan, nik, judul_keluhan, isi_keluhan, tanggal_keluhan) 
            VALUES (?, ?, ?, ?, ?)"
        );
...
<div class="container">
    ...
    <div class="main-content">
        <header><h1>Form Keluhan</h1></header>
        ...
        <div class="card form-card">
            ...
            <form method="post" action="ikel.php" class="input-form">

                <!-- Hidden otomatis -->
                <input type="hidden" name="tanggal_keluhan" value="<?= date('Y-m-d'); ?>">
                ...
                <div class="form-btn-group">
                    <button type="submit" name="add_keluhan" class="btn">Submit</button>
